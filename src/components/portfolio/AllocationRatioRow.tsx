  ...
  if (!latest) {
    return (
      <div className={styleMr(styles.rowWrapper)}>
        ...
        <CreateButton meta={{ portfolioId: portfolioId }} resource={'profit_allocation_ratios'} size="small">
          设置分配比例
        </CreateButton>
      </div>
    );
    ...
    return (
      <div className={styleMr(styles.rowWrapper)}>
        <Descriptions title={'收益分配'}>
          <Descriptions.Item label={'用户'}>{latest.toUserRatio / 100}%</Descriptions.Item>
          <Descriptions.Item label={'平台'}>{latest.toPlatformRatio / 100}%</Descriptions.Item>
          <Descriptions.Item label={'团队'}>{latest.toTeamRatio / 100}%</Descriptions.Item>
          <Descriptions.Item label={'生效时间'}>{formatDatetime(latest.createdAt)}</Descriptions.Item>
          <Descriptions.Item label={'参数版本'}>{latest.version}</Descriptions.Item>
        </Descriptions>
