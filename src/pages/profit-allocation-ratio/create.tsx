...
export const CreateProfitAllocationRatio = () => {
  ...
  const onCreate = useCallback(
    ...
    [mutate, messageApi, list],
  );

  return (
    <Create saveButtonProps={saveButtonProps}>
      {contextHolder}
      <Form {...formProps} layout="vertical" onFinish={onCreate}>
        ...
        >
          {portfolioId == undefined ? <Select options={options} style={{ width: '250px' }} /> : <Input />}
        </Form.Item>

        <Form.Item label={'用户比例'} name={'toUser'} rules={[{ required: true }]}>
          <InputNumber precision={2} max={100} suffix={'%'} style={{ width: '250px' }} />
        </Form.Item>

        <Form.Item label={'平台比例'} name={'toPlatform'} rules={[{ required: true }]}>
          <InputNumber precision={2} max={100} suffix={'%'} style={{ width: '250px' }} />
        </Form.Item>

        <Form.Item label={'团队比例'} name={'toTeam'} rules={[{ required: true }]}>
          <InputNumber precision={2} max={100} suffix={'%'} style={{ width: '250px' }} />
        </Form.Item>
      </Form>
    </Create>
